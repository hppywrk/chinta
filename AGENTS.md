...
- `/chinta-auth`: working Python service (`test_token_cache_headers.py` regression tests)
...
- Auth has a small pytest suite (`chinta-auth/test_token_cache_headers.py`); no README, CONTRIBUTING guide, or Makefile are currently present.
- Token responses from `/authenticate` and `/auth/callback` must send `Cache-Control: no-store` (RFC 6749 §5.1).
